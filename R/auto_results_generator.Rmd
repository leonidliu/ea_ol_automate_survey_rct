...
if ("dv_confirm_jackson" %in% dvs$dv_name) {
  
  df <- df %>%
    mutate(across(c(confirm_jackson_20220321_dv,
                    confirm_jackson_20220321_dv_push),
                  list(`recoded` = 
                         ~case_when(substr(., 1, 1) == "1" ~ 1,
                                    substr(., 1, 1) == "2" ~ 1,
                                    substr(., 1, 1) == "3" ~ 0,
                                    substr(., 1, 1) == "4" ~ 0,
                                    TRUE ~ NA_real_)))) %>%
    mutate(dv_confirm_jackson =
             coalesce(confirm_jackson_20220321_dv_recoded,
                      confirm_jackson_20220321_dv_push_recoded)) %>%
    select(-ends_with("recoded"))
  
}

if ("dv_jackson_qualified" %in% dvs$dv_name) {
  
  df <- df %>%
    mutate(dv_jackson_qualified = 
             case_when(substr(judge_jackson_qualified_20220321_dv, 1, 1) == "1" ~ 1,
                       substr(judge_jackson_qualified_20220321_dv, 1, 1) == "2" ~ 1,
                       substr(judge_jackson_qualified_20220321_dv, 1, 1) == "3" ~ 0,
                       substr(judge_jackson_qualified_20220321_dv, 1, 1) == "4" ~ 0,
                       substr(judge_jackson_qualified_20220321_dv, 1, 1) == "5" ~ 0,
                       TRUE ~ NA_real_))
